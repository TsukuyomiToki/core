    ...
    , m_wayland(false)
{
    ...
    QDBusConnection::sessionBus().registerService(QStringLiteral("com.pisces.Session"));
    QDBusConnection::sessionBus().registerObject(QStringLiteral("/Session"), this);

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("Pisces Session"));
...
void Application::initEnvironments()
{
    ...
    // Environment
    qputenv("DESKTOP_SESSION", "Pisces");
    qputenv("XDG_CURRENT_DESKTOP", "Pisces");
    qputenv("XDG_SESSION_DESKTOP", "Pisces");

    // Qt
    qputenv("QT_QPA_PLATFORMTHEME", "pisces");
    qputenv("QT_PLATFORM_PLUGIN", "pisces");
...
{
    QSettings settings(QSettings::UserScope, "piscesys", "language");
...
void Application::initKWinConfig()
{
    ...
    settings.setValue("library", "org.pisces.decoration");
...
// But it won't work if piscesDE is not started by systemd.
...
{
    // bool isPiscesys = QFile::exists("/etc/piscesys");
