...
ThemeManager::ThemeManager(QObject *parent)
    : QObject(parent)
    , m_settings(new QSettings(QStringLiteral("piscesys"), QStringLiteral("theme")))
{
    ...
    m_wallpaperPath = m_settings->value("Wallpaper", "/usr/share/backgrounds/piscesys/default.jpg").toString();
    ...
    if (!m_settings->contains(s_systemFontName)) {
        QSettings lanSettings(QStringLiteral("piscesys"), QStringLiteral("language"));
...
void ThemeManager::setDevicePixelRatio(qreal ratio)
{
    ...
    QProcess p;
    p.setProgram("pkexec");
    p.setArguments(QStringList() << "pisces-sddm-helper"
...
void ThemeManager::updateGtk3Config()
{
    ...
    // theme
    settings.setValue("gtk-theme-name", isDarkMode() ? "Pisces-dark" : "Pisces-light");
    settings.sync();
}
...
void ThemeManager::updateFontConfig()
{
    ...
    QFile file(targetPath + "/99-pisces.conf");
